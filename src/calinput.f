...
!     
      elseif(textpart(1)(1:8).eq.'*ENDSTEP') then
        call save_step_loads(istep,nforc,nodeforc,ndirforc,xforc,
     &       iamforc,nforc_,nload,nelemload,sideload,xload,iamload,
     &       nload_,nbody,cbody,ibody,xbody,xbodyold,nbody_,nam)
        exit
!     
...
!     
      elseif(textpart(1)(1:24).eq.'*USERBEAMDESIGNOVERRIDES' .or.
     &       textpart(1)(1:23).eq.'*USERBEAMDESIGNOVERRIDE') then
        call userbeamdesignoverrides(inpc,textpart,set,istartset,
     &       iendset,ialset,nset,lakon,ne,irstrt,istep,istat,n,key,
     &       iline,ipol,inl,ipoinp,inp,ipoinpc,ier)
!     
      elseif(textpart(1)(1:15).eq.'*USERBEAMDESIGN') then
        call userbeamdesign(inpc,textpart,set,istartset,iendset,
     &       ialset,nset,lakon,ne,irstrt,istep,istat,n,key,iline,
     &       ipol,inl,ipoinp,inp,ipoinpc,ier)
!     
      elseif(textpart(1)(1:14).eq.'*USERBEAMCHECK') then
        call userbeamcheck(inpc,textpart,set,istartset,iendset,
     &       ialset,nset,lakon,ne,irstrt,istep,istat,n,key,iline,
     &       ipol,inl,ipoinp,inp,ipoinpc,jobnamec,ier)
!     
      elseif(textpart(1)(1:20).eq.'*USERLOADCOMBINATION' .or.
     &       textpart(1)(1:16).eq.'*USERCOMBINATION') then
        call userloadcombinations(inpc,textpart,istep,istat,n,key,
