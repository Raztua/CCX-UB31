...
!     
      real*8 prop(*)
!     
!     Initialize all allocated properties to 0.d0 to prevent reading garbage/uninitialized data
      do i = nprop + 1, nprop_
         prop(i) = 0.d0
      enddo
!
...
c      enddo
     ...
     &       "*USER SECTION%",ier)
      ...
      iset=i
!     
!     Set npropstart to max(1, nprop) so the pointer is at least 1, avoiding unassigned 0-pointer bug for first user section
